...
class Lib {
    ...
    function validaEmail($email) {
        ...
        while ($i < $tamanho) {
            if ($email[$i] == '@')
                $teste1 = true; //Verifica se existe um @
            if ($email[$i] == '.')
                $teste2 = true; //Verifica se existe um ponto
            $cont++;
            $i++;
        }

        if (($teste1 == false) || ($cont <= 10) || ($teste2 == false))
            return false;
        return true;
    }
    ...
    function valida_data($data) {
        ...
        if ($tamanho != 10)
            return FALSE; //Caso não esteja no formato dd/mm/aaaa
        if (($ano < 1900) || ($ano > 2011))
            return FALSE; //Alterar para pegar o ano do sistema
        if (($mes < 1) || ($mes > 12))
            return FALSE;
        if (($dia < 1) || ($dia > 31))
            return FALSE;
        ...
        switch ($mes) {
            ...
            case "11":
                if ($dia > 30)
                    return FALSE; //Meses: 4,6,9 e 11 têm 30 dias                           
                break;
